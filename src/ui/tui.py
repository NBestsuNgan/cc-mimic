...
class TUI:
    ...
    def _extract_read_file_code(self, text: str) -> tuple[int, str] | None:
        ...
        for line in body.splitlines():
            # 1| def main():
            # ^ at the begginning of line
            # \s there are space
            # * which may be have many of it
            # (\d+) follow by many number
            # \| and ignore speical character pipe meaning as text
            # . stand for character -> then we catch (.*) which mean we catch all any character after line number and pipe
    ...
    def tool_call_complete(
        ...
        if name == "read_file" and success:
            ...
            if isinstance(key, str) and key:
                summary.append(key)
            if isinstance(found, bool) and found is not None:  # and found is not None case of delete
...
- `/pruning [force]` - Clear old tool results to free up context
