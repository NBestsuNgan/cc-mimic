...
class TodosTool(Tool):
    name = "todos"
    description = "Manage a task list for the current session. Use this to track progress on multi-step task."
    kind = ToolKind.MEMORY
    schema = TodosParams

    def __init__(self, config: Config) -> None:
        super().__init__(config)
        self._todos: dict[str, str] = {} # "ID": "content"
    ...
    async def execute(self, invocation: ToolInvocation) -> ToolResult:
        ...
        if params.action.lower() == "add":
            ...
            return ToolResult.success_result(f"Added to [{todo_id}: {params.content}]")
